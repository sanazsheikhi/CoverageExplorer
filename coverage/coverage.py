...
class Coverage():
    ...
    def __init__(self, file_name=None, data_list=None, sd=None,
        ...
        self.sd = sd
        self.dimensions = 2 # This should be before calling read_file
...
def plot(fname):
    ...
    for traj in trajectories_:
        count_ += 1
        if fname.find("ACASXU") != -1:
            trajectories.append(traj[:, 3:5])
        else:
...
def main():
    ...
    if config['sd']:   sd_list = config['sd']
