...
class MPC:
    ...
    def __init__(self, sim_states_int, sim_cmds_int):
        s = time.time()
        index = int(len(sim_states_int) * 1)
        ...
        # print(f"self.A_DMD {self.A_DMD}")
        # print(f"self.B_DMD {self.B_DMD}")

        print("max A: ", np.amax(self.A_DMD))

        self.T = 5# Time window, look ahead
        ...
        e = time.time()
        print(f"Koopman model construction time {e-s}")
...
#----------------------------------------------------------------------------------------------
    ...
    def predict(self, current_state, target_state):
        ...
        'For generate_xp we use full state (speed, rpm, gear) because koopman had been trained on this'
        ...
        for t in range(self.T):
            ...
            # dist_func = x[0:3, [t+1]] - z_[0:3]
            dist_func = x[0:2, [t+1]] - z_[0:2]
            cost += cp.quad_form(dist_func, Q)
        # dist_func = x[0:3, [self.T]] - z_[0:3]
        # cost += cp.quad_form(dist_func, Q)

            # cost += (x[0:1, t:t + 1] - z_[0:1]) * (x[0:1, t:t + 1] - z_[0:1]) # does not accept sqrt
            #cost += (x[0,t+1] - target_state[0])**2 + (x[1,t+1] - target_state[1])**2
            #cost += (x[0,t] - target_state[0])**2 + (x[1,t] - target_state[1])**2
        ...
        """ For estimating the cost we take the last state in 
            ...
            predicitons for the whole time window
        """
        ...
        #dist_func = x[0:2, self.T-1:self.T] - target_state[0:2]
        #cost += cp.quad_form(dist_func, Q)

        # constr += [-3.0 / self.km.norm_c <= u[:, :]]
        # constr += [u[:, :] <= 3.0 / self.km.norm_c]

        constr += [0.0 <= u[0, :]]
        constr += [u[0, :] <= 100.0 / self.km.norm_c_0]  #throttle
        constr += [0.0 <= u[1, :]]
        constr += [u[1, :] <= 350.0 / self.km.norm_c_1]  #brake
        ...
        #print("optimal value with ECOS:", problem.value)self.norm_s_t
        ...
        if problem.status == cp.OPTIMAL or problem.status == cp.OPTIMAL_INACCURATE:
            ...
            answers.append(f)
            #action = u[0, 0].value * self.km.norm_c
            action[0] = u[0, 0].value  * self.km.norm_c_0
            action[1] = u[1, 0].value  * self.km.norm_c_1
        ...
        return action, ps, X_ #, X_[0, 1], X_[1, 1], X_[2, 1]
