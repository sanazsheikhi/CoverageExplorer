...
class State():
    ...
    def simulate(self, cmd_list):
        ...
        while t + 1e-6 < tmax:
            ...
            ownship_state.append([self.vec[0], self.vec[1], self.vec[2]])
        ...
        if self.min_dist <= 500: 
            collision = True
            print("!!!! Collision at distance  ", self.min_dist)

        deviation = False
        # If the ownship x positions changes at least 2 units from origin I take it as deviation
        if self.vec[0] > 2 or  self.vec[0] < -2:
            deviation = True
            print("deviation")


        return np.array(ownship_state)
...
def plot(s, save_mp4=False):
    ...
    # Sanaz
    plt.xlabel('X (ft)', fontsize=16, fontweight='bold')
    plt.ylabel('Y (ft)', fontsize=16, fontweight='bold')
    ...
    if save_mp4:
        ...
        # plt.savefig('plot.png')
        # plt.savefig('plot_koopman.pdf', format="pdf")
        plt.show()
...
def make_random_input(seed, num_inputs=100):
    ...
    for _ in range(num_inputs):
        ...
        #cmd_list.append(np.random.randint(2)) 

        #nums = [0, 1, 2, 3, 4]
        #c = np.random.choice(nums)
        #cmd_list.append(c)
...
def run_acasxu(sim_count=100, steps=120, dt_=0.01):
    ...
    while len(all_traj_states) < sim_count:
        ...
        print(f"run {seed}")
        init_vec, cmd_list = make_random_input(seed, steps)
        seed += 1
        ...
        states_own = s.simulate(cmd_list)

        # reject simulations where the minimum distance was near the start
        if s.vec[-1] < 3.0:
            continue

        all_traj_states.append(states_own)

        # plot(s, False)

    # 'Storing the ownship trajectory for coverage'
    # fname = 'ownship_states_random_' + str(seed) + '.pkl'
    # with open(fname, 'wb') as fhandle:
    #     pickle.dump(sim_states, fhandle)
