...
class KDNode(Node):
    ...
    def axis_dist(self, point, axis):
        ...
        """
        #return math.pow(self.data[axis] - point[axis], 2) #commented by snz for math range error of pow()
...
def check_dimensionality(point_list, dimensions=None):
    ...
    for p in point_list:
        if len(p) != dimensions:
            raise ValueError(f'All Points in the point_list must have the same dimensionality.' + str(len(p)))
